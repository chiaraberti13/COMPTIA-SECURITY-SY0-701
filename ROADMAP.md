...
- [ ] 🟡 **P1 — Metadati di revisione dei contenuti:** registro per obiettivo in `src/contentReview.ts` con `status` (`reviewed`/`needs-review`/`deprecated`), `lastReviewed`, `reviewer` e `sources`, senza riscrivere i dataset (come per gli obiettivi delle domande). Mostrato in ogni guida ("Fonti e revisione") e in `docs/coverage-matrix.md`; `tests/contentReview.test.ts` rifiuta "reviewed" senza data e revisore. Tutti i 28 obiettivi partono onestamente da `needs-review`: manca la revisione umana, obiettivo per obiettivo — 2026-09-26. Resta il livello della singola domanda o voce di glossario, se servirà. **M**
...
- [x] **P0 — Matrice di copertura per obiettivo pubblicata:** `docs/coverage-matrix.md`, collegata dai README — 2026-09-24. Dal 2026-09-26 ha per ogni obiettivo lo stato di revisione (con data e revisore quando c'è) e le fonti, più il catalogo delle fonti primarie e secondarie, i cui link sono verificati dal controllo settimanale dei link esterni.
...
- [ ] 🟡 **P0 — Citazioni verificabili:** fonti verificabili a livello di obiettivo, con la data dell'assegnazione (2026-09-26) e i link controllati ogni settimana; restano le citazioni puntuali per singola affermazione normativa o dato che cambia nel tempo.
