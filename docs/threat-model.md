...
| STRIDE | Minaccia | Controlli presenti | Stato |
|---|---|---|---|
| Tampering | **Prompt injection diretta** (OWASP LLM01): l'utente chiede al modello di ignorare le regole | Il prompt di sistema dichiara che il testo dell'utente è un dato e non un'istruzione, e ha un test. Il danno possibile è limitato perché il modello non ha strumenti, non legge dati riservati e la sua risposta è solo testo mostrato a chi l'ha chiesta | 🟡 nessun filtro è completo contro l'injection; il rischio residuo è accettato perché l'impatto è basso |
| Tampering | Output del modello malformato o ostile nella remediation (OWASP LLM05) | `validateRemediationPayload` accetta solo 3 domande con 4 opzioni e un indice valido; gli ID li assegna il server; in caso contrario → 502. Test dedicato | ✅ |
...
| Tampering | Pacchetto npm compromesso o appena pubblicato | `package-lock.json` con `npm ci`; Dependabot con attesa di 7 giorni; policy "versioni pubblicate da almeno 7 giorni" in `CONTRIBUTING.md`; `npm audit`, dependency review e CodeQL in `security.yml`; `overrides` per le dipendenze transitive vulnerabili | ✅ |
